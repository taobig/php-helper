...
use taobig\helpers\http\Request;
...
class RequestTest extends \TestCase
{
    ...
    {
        $response = Request::get("localhost:8080", 10);
    ...
    public function testPostJson()
    {
        ...
        $value = 'hello';
        $headers = [$key => $value];
        $response = Request::postJson("localhost:8080/json", $params, 10, ["headers" => $headers]);
        $content = $response->getBody()->getContents();
    ...
    public function testPostForm()
    {
        $time = time();
        $params = ['time' => $time];
        $response = Request::postForm("localhost:8080/form", $params, 10);
        $content = $response->getBody()->getContents();
        $this->assertSame("string", gettype($content));
    ...
    public function testPostFile()
    {
        ...
        $response = Request::postFile("localhost:8080/file", $params, 10);
        $content = $response->getBody()->getContents();
        $this->assertSame("string", gettype($content));
